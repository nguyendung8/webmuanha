<?php
   ...
   if(isset($_POST['add_room'])){//thêm sách mới từ submit form name='add_room'
      ...
      $select_product_name = mysqli_query($conn, "SELECT name FROM `houses` WHERE name = '$name'") or die('query failed');//truy vấn kiểm tra nhà đã tồn tại chưa

      if(mysqli_num_rows($select_product_name) > 0){
         $message[] = 'nhà đã tồn tại.';
      }else{//chưa thì thêm mới
         $add_room_query = mysqli_query($conn, "INSERT INTO `houses`(name, cate_id, location,area, price, rate, description, image) VALUES('$name', '$category', '$location', '$area', '$price', '$rate', '$description', '$image')") or die('query failed');
         if($add_room_query){
            ...
            $message[] = 'Thêm nhà không thành công!';
         }
      }
   }
   ...
   if(isset($_GET['delete'])){//xóa sách từ onclick <a></a> href='delete'
      ...
      $delete_image_query = mysqli_query($conn, "SELECT image FROM `houses` WHERE id = '$delete_id'") or die('query failed');
      $fetch_delete_image = mysqli_fetch_assoc($delete_image_query);
      unlink('uploaded_img/'.$fetch_delete_image['image']);//xóa file ảnh của sách cần xóa
      mysqli_query($conn, "DELETE FROM `houses` WHERE id = '$delete_id'") or die('query failed');
   ...
   if(isset($_POST['update_product'])){//cập nhật sách từ form submit name='update_product'
      ...
      mysqli_query($conn, "UPDATE `houses` SET name = '$update_name', location = '$update_location', cate_id='$update_category', price='$update_price' WHERE id = '$update_p_id'") or die('query failed');
...
<section class="add-products">
   ...
   <h1 class="title">Nhà</h1>

   <form action="" method="post" enctype="multipart/form-data">
      <h3>Thêm nhà</h3>
      <input type="text" name="name" class="box" placeholder="Tên nhà" required>
      <select name="category" class="box">
         <?php
            ...
            if(mysqli_num_rows($select_category)>0){
               ...
               echo "<option>Không có danh mục nhà nào.</option>";
            }
         ?>
      </select>
      <input type="text" name="location" class="box" placeholder="Địa chỉ" required>
      <input type="text" name="area" class="box" placeholder="Diện tích" required>
      <input type="text" name="price" class="box" placeholder="Giá nhà" required>
...
<section class="show-products">
   ...
   <div class="box-container">
      ...
      <?php
         $select_products = mysqli_query($conn, "SELECT * FROM `houses`") or die('query failed');
      ...
      ?>
               <div style="height: -webkit-fill-available;" class="box">
                  ...
                  <a href="admin_rooms.php?delete=<?php echo $fetch_products['id']; ?>" class="delete-btn" onclick="return confirm('Xóa nhà này?');">Xóa</a>
               </div>
      <?php
            }
      }else{
         echo '<p class="empty">Không có nhà nào được thêm!</p>';
      }
      ?>
   </div>
...
<section class="edit-product-form">
   ...
   <?php
      if(isset($_GET['update'])){//hiện form update từ onclick <a></a> href='update'
         ...
         $update_query = mysqli_query($conn, "SELECT * FROM `houses` WHERE id = '$update_id'") or die('query failed');
   ...
   ?>
               <form action="" method="post" enctype="multipart/form-data">
                  ...
                  <input type="text" name="update_price" value="<?php echo $fetch_update['price']; ?>" class="box" required placeholder="Giá">
